...
class BMA4:
    ...
    def write_config_file(self):
       ...
       self.streamindex=0x0000
       buf = fw.bma423_config_file[self.streamindex:self.streamindex + 32]
       nread = 32
       while nread > 0:
         self.stream_transfer_write(memoryview(buf))
         buf = fw.bma423_config_file[self.streamindex:self.streamindex + 32]
         nread += 32
         if nread >= BMA4_CONFIG_STREAM_SIZE:
            nread = 0
       self.write_byte(BMA4_INIT_CTRL_ADDR, 0x01);
       time.sleep_ms(160)
       asic=self.read_byte(BMA4_INTERNAL_STAT)
    ...
    @property
    def accel_odf(self):
       odf=self.read_byte(BMA4_ACCEL_CONFIG_ADDR) & 0xF
       return odf
    
    @accel_odf.setter 
    def accel_odf(self,val):
       val = val & 0xF
       if val <= 12:
           tmp=self.read_byte(BMA4_ACCEL_CONFIG_ADDR) & 0xF
           tmp |= val
           self.write_byte(BMA4_ACCEL_CONFIG_ADDR,tmp)
       
    @property
    def accel_perf(self):
       perf=self.read_byte(BMA4_ACCEL_CONFIG_ADDR)
       perf = (perf & 0x80) >> 7
       return perf
    ...
    def set_remap_axes(self, remap_data):
        data=self.read_data(BMA4_FEATURE_CONFIG_ADDR, BMA423_FEATURE_SIZE)
        index = BMA423_AXES_REMAP_OFFSET;
        x_axis = remap_data[0] & BMA423_X_AXIS_MASK;
        x_axis_sign = (remap_data[1] << 2) & BMA423_X_AXIS_SIGN_MASK;
        y_axis = (remap_data[2] << 3) & BMA423_Y_AXIS_MASK;
        y_axis_sign = (remap_data[3] << 5) & BMA423_Y_AXIS_SIGN_MASK;
        z_axis = (remap_data[4] << 6) & BMA423_Z_AXIS_MASK;
        data[index] = x_axis | x_axis_sign | y_axis | y_axis_sign | z_axis
        data[index + 1] = remap_data[5] & BMA423_Z_AXIS_SIGN_MASK
        self.write_data(BMA4_FEATURE_CONFIG_ADDR,data)
...
class BMA423(BMA4):
        def __init__(self, i2c, address=BMA4_I2C_ADDR_SECONDARY):
            ...
            if self.chip_id != BMA423_CHIP_ID:
                ...
                self.write_byte(0x7E, 0xB6) #reset
                time.sleep_ms(1)
                self.write_byte(0x5E, 0x00) #init
                self.resolution = 12
                self.feature_len = BMA423_FEATURE_SIZE
                self.variant = 1
                #self.write_config_file()
                super(BMA423,self).write_config_file()
                self.feature_config(BMA423_WRIST_SC_PARAM)
                time.sleep_ms(150)
                self.write_byte(BMA4_INT1_IO_CTRL_ADDR,10) # level trigger, output high enable, push
                self.write_byte(BMA4_INT2_IO_CTRL_ADDR,5)
                self.write_byte(BMA4_INTR_LATCH_ADDR,0)
